...
import sf.wicklet.gwt.client.dsl.Attributes;
import sf.wicklet.gwt.client.dsl.DOCTYPE;
...
public abstract class XHtmlBuilderBase<E extends IElement> implements IXHtmlBuilder<E> {
	...
	public IPI xml(final String encoding) {
		return new PI("xml", "encoding=" + XmlUtil.quoteAttr(encoding));
	}

	@Override
	public IPI xml(final String version, final String encoding) {
		return new PI(
			"xml", "version=" + XmlUtil.quoteAttr(version), "encoding=" + XmlUtil.quoteAttr(encoding));
	}
	...
	public E style(final String...content) {
		return elm(
			"style",
			new Text(XmlUtil.escText(GwtTextUtil.join(GwtTextUtil.getLineSeparator(), content)).toString()));
	}
	...
	public IText esc(final String...content) {
		...
		for (int i = 0; i < len; ++i) {
			c[i] = XmlUtil.escText(content[i]).toString();
		}
		return new Text(c);
	}
	...
	public IPI pi(final String target, final String...content) {
		return new PI(target, GwtTextUtil.join(GwtTextUtil.getLineSeparator(), content));
	}
	...
	public IAttribute css(final Object...values) {
		return new Attribute("class", GwtTextUtil.joins(" ", values));
	}
	...
	public IAttribute istyle(final Object...styles) {
		return attr("style", GwtTextUtil.joins(" ", styles));
	}
