...
import sf.wicklet.dsl.html.api.TAG;
...
public class XHtmlSerializer<T extends PrintWriter> implements INodeVisitor<T> {

	//////////////////////////////////////////////////////////////////////

	protected IXmlUtil xmlUtil = new XmlUtil();
	...
	public static String serialize(
		...
		e.accept(new XHtmlSerializer<StringPrintWriter>(tab).indent(indent).noEmptyTag(noemptytag), w);
		return w.toString();
	}
	...
	public XHtmlSerializer(final String tab) {
		this.tab = tab;
	}

	//////////////////////////////////////////////////////////////////////

	public XHtmlSerializer<T> indent(final String indent) {
		this.indent = indent;
		return this;
	}

	public XHtmlSerializer<T> tab(final String tab) {
		this.tab = tab;
		return this;
	}

	public XHtmlSerializer<T> noEmptyTag(final boolean noemptytag) {
	...
	@Override
	public void visit(final IWicketComponent node, final T data) {
		visitElement(node, data);
	}

	@Override
	public void visit(final IWicketElement node, final T data) {
		visitElement(node, data);
	}
	...
	protected void attributes(final T w, final IElement node) {
		for (final IAttribute a: node.attributes()) {
			if (Attribute.LB == a) {
				...
				line.append("=");
				line.append('"');
				line.append(xmlUtil.escAttr(a.avalue()));
				line.append('"');
	}}}
	...
	protected void visitElement(final IElement node, final T data) {
		...
		if (tag != null && tag.isBlockFormat() || node instanceof IWicketElement) {
			if (!preformatted) {
				lb(data);
			}
	...
	protected void content(final String[] content) {
		...
		if (len == 1) {
			...
			line.append(TextUtil.join(getindent(), content));
	}}
	...
	private String getindent() {
		final StringBuilder b = new StringBuilder(TextUtil.getLineSeparator());
		b.append(indent);
		if (tab != null) {
			int level = level();
			if (level > startLevel) {
